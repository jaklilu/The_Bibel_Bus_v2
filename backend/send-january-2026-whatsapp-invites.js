// Script to send WhatsApp invitation emails to January 2026 group members
...
db.get(`SELECT * FROM bible_groups WHERE name LIKE '%January 2026%' OR name LIKE '%Jan 2026%'`, [], async (err, group) => {
  if (err) {
  ...
  if (!group) {
    ...
    db.all('SELECT id, name FROM bible_groups ORDER BY start_date DESC LIMIT 10', [], (err, groups) => {
      if (!err) {
