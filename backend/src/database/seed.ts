...
const seedDatabase = async () => {
  try {
    ...
    const adminUser = await runQuery(
      ...
      ['Bible Bus Admin', 'JayTheBibleBus@gmail.com', '(555) 000-0000', adminPasswordHash, 'admin']
    )
    ...
    for (const user of users) {
      await runQuery(
        'INSERT OR IGNORE INTO users (name, email, phone, password_hash, city, mailing_address) VALUES (?, ?, ?, ?, ?, ?)',
        [user.name, user.email, user.phone, user.password_hash, user.city, user.mailing_address]
      )
    }
    ...
    for (const group of groups) {
      await runQuery(
        'INSERT OR IGNORE INTO bible_groups (name, start_date, end_date, registration_deadline, max_members, status) VALUES (?, ?, ?, ?, ?, ?)',
        [group.name, group.start_date, group.end_date, group.registration_deadline, group.max_members, group.status]
      )
    }
    ...
    for (const member of groupMembers) {
      await runQuery(
        'INSERT OR IGNORE INTO group_members (group_id, user_id, join_date) VALUES (?, ?, ?)',
        [member.group_id, member.user_id, member.join_date]
      )
    }
    ...
    for (const message of adminMessages) {
      await runQuery(
        'INSERT OR IGNORE INTO admin_messages (title, message, type, target_group_id, created_by) VALUES (?, ?, ?, ?, ?)',
        [message.title, message.message, message.type, message.target_group_id, message.created_by]
      )
    }
